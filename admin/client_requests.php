...
?>
    <div class="card">
        <div class="card-header">
            All Clients Requests (If Viewing On Phone Or Tablet, Click + Or - Respectively To Expand Or Collapse Rows)
        </div>
        <div class="card-body">
            <button type="button" class="btn btn-success mb-3" data-bs-toggle="modal" data-bs-target="#addClientRequest">
                Add Client Request
            </button>
            <div class="modal fade" id="addClientRequest" tabindex="-1" aria-labelledby="addClientRequestLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Add Client Request</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="/actions.php" method="POST" onsubmit="return confirm('Are you sure you want to go ahead and add this client request?');">
                            <div class="modal-body">
                                <input type="hidden" name="action" value="add_client_request">
                                <div class="row">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select form-select-md mb-3" aria-label=".form-select-lg example" name="clientRequestReasonId" required>
                                                <?php foreach ($clientRequestReasons as $clientRequestReason) { ?>
                                                    <option value="<?= $clientRequestReason['id']; ?>"><?= $clientRequestReason['name']; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6 col-lg-6 col-xs-12">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select form-select-md mb-3" aria-label=".form-select-lg example" name="clientRequestOutcomeId" required>
                                                <?php foreach ($clientRequestOutcomes as $clientRequestOutcome) { ?>
                                                    <option value="<?= $clientRequestOutcome['id']; ?>"><?= $clientRequestOutcome['name']; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select form-select-md mb-3" aria-label=".form-select-lg example" name="clientRequestReferralId" required>
                                                <?php foreach ($clientRequestReferrals as $clientRequestReferral) { ?>
                                                    <option value="<?= $clientRequestReferral['id']; ?>"><?= $clientRequestReferral['name']; ?></option>
                                                <?php } ?>
                                            </select>                                        
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6 col-lg-6 col-xs-12">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select form-select-md mb-3" aria-label=".form-select-lg example" name="signPostId" required>
                                                <?php foreach ($signPosts as $signPost) { ?>
                                                    <option value="<?= $signPost['id']; ?>"><?= $signPost['name']; ?></option>
                                                <?php } ?>
                                            </select>                                        
                                        </div>
                                    </div>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" name="editSubmit" class="btn btn-primary">Add Client Request</button>
                            </div>
                        </form>
                    </div>
                </div>
